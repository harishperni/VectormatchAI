...
DATE_RANGE_PATTERNS = [
    ...
    re.compile(
        r"(?P<smo>\d{1,2})/(?P<syr>\d{4})\s*[-–—to]+\s*(?P<emo>\d{1,2}|Present|Current|Now|Today)\s*/?(?P<eyr>\d{4})?",
        re.IGNORECASE,
    ),
    re.compile(
        r"(?P<syr>\d{4})\s*[-–—to]+\s*(?P<eyr>\d{4}|Present|Current|Now|Today)",
        re.IGNORECASE,
    ),
]
...
def parse_resume_with_llm(text: str) -> dict[str, Any] | None:
    ...
    print("==== WORK EXCERPT ====")
    print(work_excerpt)
    job_blocks = extract_job_blocks(work_excerpt)
    print("==== JOB BLOCKS ====")
    ...
    print("==== PYTHON EXPERIENCE ENTRIES ====")
    print(json.dumps(python_entries, indent=2))

    print(extract_experience_entries_from_blocks(work_excerpt))
    claimed_years = _extract_explicit_experience_years(text)
    print("==== WORK EXCERPT LINES ====")
    for idx, line in enumerate(work_excerpt.splitlines(), 1):
        print(idx, repr(line))
    ...
    system_prompt = (
        ...
        "9) If Python-detected entries look valid, preserve them unless clearly wrong.\n"
        "10) current_last_job must be title only.\n"
        "11) candidate_location must be US-only in format 'City, ST'; if uncertain or non-US, return null."
    )
    ...
    # Always trust Python-detected experience entries for long resumes
    if python_entries:
...
def _extract_explicit_experience_years(text: str) -> float | None:
    ...
    for line in lines[:60]:
        lowered = line.lower()
        if any(noise in lowered for noise in EXPLICIT_NOISE_WORDS):
            continue
        for match in EXPLICIT_EXPERIENCE_PATTERN.finditer(line):
